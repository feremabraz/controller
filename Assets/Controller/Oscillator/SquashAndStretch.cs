...
public class SquashAndStretch : MonoBehaviour
{
    ...
    [SerializeField] private float conversion = 0.15f;
    ...
    {
        var rawOscillatorContribution = GetContributionFromOscillator();
        var oscillatorContribution = rawOscillatorContribution;
        if (constantVolume)
        {
            oscillatorContribution = MaintainVolume(rawOscillatorContribution);
        }
        var localScale = _localEquilibriumScale + oscillatorContribution;
        return (localScale);
    }
    ...
    {
        var oscillatorContribution = oscillator.transform.localPosition * conversion;
    ...
    private Vector3 MaintainVolume(Vector3 primaryDeformation)
    {
        ...
        {
            var rightCrossSecondaryDeformation = Vector3.Cross(primaryDeformation, transform.right);
            var upCrossSecondaryDeformation = Vector3.Cross(primaryDeformation, transform.up);
            var forwardCrossSecondaryDeformation = Vector3.Cross(primaryDeformation, transform.forward);
            var totalDeformation = primaryDeformation + 0.5f * (rightCrossSecondaryDeformation + upCrossSecondaryDeformation + forwardCrossSecondaryDeformation);
